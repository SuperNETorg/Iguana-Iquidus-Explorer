              ...
              lib.syncLoop(vin.length, function (loop) {
                ...
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  lib.calculate_total(vout, function(total){
                      if(tx.timestamp && !tx.time){tx.time=tx.timestamp;}
                  console.log("txid "+tx.txid+" vin "+vin.toString()+" total "+total+" timestamp "+tx.time+" blockhash "+tx.blockhash+" block.height "+block.height);
                    var newTx = new Tx({
                      txid: tx.txid,
                      vin: nvin,
                      vout: vout,
      ...
      lib.get_blockhash(start + x, function(blockhash){
        if (blockhash) {
          lib.get_block(blockhash, function(block) {
            if (block) {
                console.log("blockhash "+blockhash+ " block "+block);
